...
def main() -> None:
    ...
    logging.info("Loading buyer model from %s", cfg.paths.checkpoints.grpo_poker)
    buyer_tokenizer = AutoTokenizer.from_pretrained(cfg.paths.checkpoints.grpo_poker)
    ...
    buyer_model = AutoModelForCausalLM.from_pretrained(
        cfg.paths.checkpoints.grpo_poker, dtype=dtype, device_map="auto"
    )
    buyer_model.eval()
    ...
    if returns:
        ...
        print(f"  Average  : {sum(returns)/len(returns):.3f}x")
        print(f"  Best     : {max(returns):.3f}x")
        print(f"  Worst    : {min(returns):.3f}x")
    else:
        print("\nNo closed deals.")
